...
20. 目的地候補行の `☆` でお気に入り保存/解除ができる
21. 目的地設定画面の先頭に `お気に入り` セクションが表示される
22. Home の `お気に入りから出発` カードに最大5件表示され、1タップでプレビューへ遷移する
23. 候補選択後にプレビュー画面が表示される
24. スタートで Driving へ遷移し、案内カードが下部、ズームUIが右上、`終了`/`メニュー` が左上縦アイコンで表示される
25. ナビ中に一定時間放置しても画面が自動スリープ/自動ロックしない
26. 案内カード右上に「速度」「全体道のり」が2段表示され、次操作地点までの距離が右上に表示されない
27. 移動中に全体道のりがリアルタイムで減少し、停止時は速度が `0.0 km/h` に収束する
28. ルート上を進むと通過済み区間のルート線が後方から消える
