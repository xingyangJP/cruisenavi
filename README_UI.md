...
項目	内容
表示	全画面 MapKit。ENCベースの海上ルート全体を表示し、スタート地点（現在地最寄りの港/海岸）とゴールを同時に表示。
情報	スタート/ゴール距離・ETA・波/風カードを掲示。
操作	「スタート」で Driving へ遷移、「キャンセル」でホームに戻る。経由地再設定もここで行う。
背景	Dark Glass + 海上ラインのグロー演出。
