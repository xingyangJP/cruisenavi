...
- フリーライド（目的地なしで自動記録のみ開始）
- 雨回避アラート（30〜60分先の降雨予測 + 回避ルート提案）
- 補給リマインド（水分・カロリー目安をナビ中に定期表示）
- 危険地点アラート（MVP: 前方急カーブ / 夜間 / 路面悪化をナビ中に通知）
- 音声ナビ（開始案内、次操作の先行案内、危険警告、到着案内）
- バックグラウンド走行中のナビ継続（位置追跡/ログ継続）
- アプリ言語切替（iOS設定の `RideLane > 言語` で日本語/英語を選択可能）
- 走行ログ一覧
- 今日の1本提案（現在地・天候に応じた1タップ開始）
- お気に入りから出発（Homeの横スクロールから1タップ開始）
- 週次ミッション（今週40kmの進捗表示）
- 休憩スポット提案（走行距離と気象リスクに応じて、休憩向きスポットを提案）
- ライド完了リワード（終了時ハイライト表示）
- ライドストーリー自動生成（ライド詳細にストーリーカード表示・共有）
